...
const int WORDCNT = 10; // change this to 20. default 512bit = 8. 10 -> 640 bits
...
#include <fstream>
// no need for a vector, we already have one;
int main(int argc, char **argv)
{
    if (argc != 3)
    {
        std::cout << "Wrong argument count. arguments should be <main> <input_file> <output_file>\n";
        return -1;
    }
    ...
    if (!inp.is_open())
    {
        std::cout << "input file " << argv[1] << " isn't accessible. aborting\n";
        inp.close();
        out.close();
        return -1;
    }
    ...
    // <=> a * a^(p-2) = 1 (mod p)
    // <=> inverse a = a^(p-2)
    bigint inverse_s = powMod(s, p - bigint(2), p);

    // Decrypt m = c2 * s^-1 (mod p)
    bigint m = mulMod(c2, inverse_s, p);
